...
class Transformer:
    ...
    def getOutput(self):
        ...
        final = Dense(16, activation='relu', kernel_regularizer=tf.keras.regularizers.l2(0.001))(average)
        final = Dropout(DROP)(final)

        return Dense(3, activation='softmax')(final)

    def buildModel(self):
        model = TFRobertaForSequenceClassification.from_pretrained(PATH + 'PhoBERT_large_transformers', num_labels=3)
        optimizer = tf.keras.optimizers.Adam(learning_rate=5e-5)
        loss = tf.keras.losses.SparseCategoricalCrossentropy(from_logits=True)
        # Xây dựng mô hình
        model.compile(optimizer=optimizer, loss=loss, metrics=['accuracy'])
        model.fit(self., np.array(y_train), batch_size=8, epochs=3, validation_split=0.2)
